...
var storeApp = (function ($) {
	...
	return {
		...
		,playSound: function(name, path) {
			path = path ? path : window.baseUrl + '/assets/harvestsnop/mp3/' + name;
...
window.jQuery(window).on("load", function () {
	...
	$("#logout").on("click", function(e) {
        ...
        .then(function(willDelete) {
            if (willDelete) {
                window.location = baseUrl+'/administrador/logout.php';
            }
        });
	});
...
function printContent(eleID, settings)
{
	...
	var html = '<html><head>'+
	'<title>'+title+'</title>'+
	'<link type="text/css" href="../assets/harvestsnop/cssmin/main.css" type="text/css" rel="stylesheet">'+
	cssLink+
